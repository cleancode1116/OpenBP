...
public class LoggingOutputStream extends OutputStream
{
	 ...
	 * associated  with this stream. The general contract of <code>close</code>   is that it closes
	 ...
	 * to be written out. The general contract of <code>flush</code> is that calling it is an
	 * indication that, if any bytes previously written have been buffered by the implementation of
	 * the output stream, such bytes should immediately be written to their intended destination.
	 */
	public void flush()
	{
		if (count == 0)
		...
		// Create String (with the appropriate length!).
		String logMessage = new String(theBytes, 0, count);
		...
		category.log(priority, logMessage);
