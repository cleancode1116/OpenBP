 ...
 */
public class SkyFileChooserUI extends BasicFileChooserUI
{
	//////////////////////////////////////////////////
	// @@ Static Members
	//////////////////////////////////////////////////

	private static final Dimension HORIZONTAL_STRUTS_5 = new Dimension(5, 1);

	private static final Dimension VERTICAL_STRUTS_5 = new Dimension(1, 5);
	...
	{
		return new SkyFileChooserUI((JFileChooser) c);
	}

	/**
	 * Creates a new SkyFileChooserUI.
	 *
	 * @see javax.swing.plaf.basic.BasicFileChooserUI#BasicFileChooserUI(JFileChooser)
	 */
	public SkyFileChooserUI(JFileChooser filechooser)
	{
		super(filechooser);
	}
	...
	public void installComponents(JFileChooser fc)
	{
		...
		bar.add(Box.createRigidArea(HORIZONTAL_STRUTS_5));
		...
		centerPanel.setBorder(new ReverseShadowBorder());
		centerPanel.setOpaque(true);
		centerPanel.setBackground(SkyTheme.COLOR_BACKGROUND_LIGHT_LIGHT);
		...
		bottomPanel.add(Box.createRigidArea(VERTICAL_STRUTS_5));
	 ...
	 * @return JPanel a panel containig a list
	 */
	protected JPanel createList(JFileChooser fc)
	{
		JPanel p = new JPanel(new BorderLayout());
		p.setBackground(SkyTheme.COLOR_BACKGROUND_LIGHT_LIGHT);
		p.setOpaque(true);
		...
		list.setBackground(SkyTheme.COLOR_BACKGROUND_LIGHT_LIGHT);
		list.setSelectionBackground(SkyTheme.COLOR_HIGHLIGHT);
	 ...
	 * @return JPanel a panel with a table
	 */
	protected JPanel createDetailsView(JFileChooser fc)
	{
		...
		detailsTable.setBackground(SkyTheme.COLOR_BACKGROUND_LIGHT_LIGHT);
		detailsTable.setSelectionBackground(SkyTheme.COLOR_HIGHLIGHT);
	...
	/**
	 * Inner class for detail view in a table.
	 */
			...
			if (col == COLUMN_FILENAME)
			{
				...
				if (f != null)
				{
					...
					if (!newDisplayName.equals(oldDisplayName))
					{
						...
						FileSystemView fsv = chooser.getFileSystemView();
						File f2 = fsv.createFileObject(f.getParentFile(), newFileName);
						if (SkyFileChooserUI.this.getModel().renameFile(f, f2))
						{
							if (fsv.isParent(chooser.getCurrentDirectory(), f2))
							{
								if (chooser.isMultiSelectionEnabled())
	...
	protected class FileRenderer extends DefaultListCellRenderer
	{
		public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus)
		{
			...
			Icon icon = getFileChooser().getFileSystemView().getSystemIcon(file);
			setIcon(icon);
